...
class AgentKinova():
    ...
    def __init__(self):
        rospy.init_node('kinova_torque_control_node')
    ...
    def robot_wrench_callback(self, msg):
        ...
        temp_torque = msg.wrench.torque.z
        if abs(temp_torque)>0.015:
            if temp_torque<0:
                temp_torque = -0.015
            else:
                temp_torque=0.015
        self.z_torque = temp_torque
    ...
    def __main__(self):
        while not rospy.is_shutdown():
            # if self.MOVING:
                CURRENT_VELOCITY = [0, 0, 0, 0, 0, 0.5-0.5/0.015*self.z_torque]
                self.velo_pub.publish(kinova_msgs.msg.PoseVelocity(*CURRENT_VELOCITY))
                self.r.sleep()
                print('==')
